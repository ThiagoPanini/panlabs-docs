...
- **Regra de derivação**: nenhum valor entra no sistema como literal. Todo valor sai de algo que já está lá, por uma operação declarada — alfa sobre um token existente (`rgb(var(--x) / n%)`), múltiplo do raio base, degrau da escala de duração — e se resolve nos dois modos como par declarado. Hex novo, px avulso ou `cubic-bezier` solto é rejeitado. É a régua de coerência do projeto, e é mecânica de propósito: uma régua de julgamento só funciona com o dono do projeto presente, que é exatamente o que a spec existe para dispensar.
