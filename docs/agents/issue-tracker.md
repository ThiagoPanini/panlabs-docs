...
As issues deste repo vivem como **issues do GitHub** em `panlabs-tech/shinydoc-docusaurus`. Use a CLI `gh` para todas as operações.

> **Este tracker não é o da org.** `panlabs-tech/.github` hospeda trabalho sobre o **padrão panlabs**; produto tem tracker próprio. O shinydoc é produto: nada dele entra lá.

## O que entra

Trabalho sobre o **shinydoc**: a spec de documentação Docusaurus, a arquitetura de tokens, o inventário de componentes, a arquitetura de informação, as pesquisas que alimentam essas decisões.
...
**Criar tickets e ligar ao mapa.** As duas APIs de relação exigem o **`id`** interno da issue, não o número — resolva com `gh api repos/panlabs-tech/shinydoc-docusaurus/issues/<numero> --jq .id`.

```bash
# ticket como sub-issue do mapa
gh api -X POST repos/panlabs-tech/shinydoc-docusaurus/issues/<MAPA>/sub_issues -F sub_issue_id=<ID_TICKET>

# ticket A bloqueado pelo ticket B
gh api -X POST repos/panlabs-tech/shinydoc-docusaurus/issues/<NUM_A>/dependencies/blocked_by -F issue_id=<ID_B>
```
...
**Listar os filhos do mapa**: `gh api repos/panlabs-tech/shinydoc-docusaurus/issues/<MAPA>/sub_issues --jq '.[] | {number, title, state, assignees: [.assignees[].login]}'`

**Ler os bloqueadores de um ticket**: `gh api repos/panlabs-tech/shinydoc-docusaurus/issues/<NUM>/dependencies/blocked_by --jq '[.[] | select(.state=="open") | .number]'`
