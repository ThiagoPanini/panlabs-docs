...
São sete ao todo, e a numeração é a da spec — não a ordem em que os arquivos aparecem. Os quatro do slice 1 estão escritos; os três restantes nascem no slice que produz a superfície que eles governam, e por isso há buraco na sequência.
...
| 4 | Contrato de estado de entrada: `outline` universal em `:focus-visible`, `:active`, piso de alvo no toque | slice 2 |
...
Os quatro escritos, nesta ordem. O ADR 1 primeiro: quem escreve CSS sem ele produz modo escuro que quebra em silêncio.
