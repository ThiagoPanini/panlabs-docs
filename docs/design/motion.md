...
A fronteira da licença **não é a landing: é a ilha**. `--sd-move-showcase` e `--sd-move-ambient` são camada 3, declarados dentro do escopo da ilha, exatamente como `--sd-glow`. `--sd-move-reveal` é o único que pertence à *rota* landing em vez da ilha, por morar no CSS Module dela.
...
CSS Modules manglam o nome do `@keyframes` que o módulo **define**; um keyframe definido num módulo é inalcançável de outro, e **a falha é silenciosa** — não anima e não avisa. Um módulo pode **referenciar** keyframe global sem problema, então a landing usa os globais e a regra não conflita com o confinamento do §5.
