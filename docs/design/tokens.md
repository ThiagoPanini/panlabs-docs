...
:root {
  ...
  --pd-optical-bold: 0 0 0.4px currentColor;
  ...
  /* O PISO DE UM CAMPO EDITÁVEL do painel de comando — a largura abaixo da
     ...
     **Ele ocupa o slot que era `--pd-api-prose-width` (577), e a troca conta
     ...
     **Ele é medida de UM componente e mora em `:root`, e a tensão é real.** A
     ...
     já resolveu isso duas vezes antes, do mesmo jeito: `--pd-copy-height` e
     `--pd-copy-menu-width` são medidas do par segmentado do cabeçalho, e
  ...
  --pd-freeze: calc(var(--pd-sidebar-width) + var(--pd-container-width));
  ...
  --pd-content-top: calc(var(--pd-sticky-top) + var(--pd-space-10));
  ...
  /* A folga lateral do shell, de cada lado. Ela dobra a partir de 997px — o
     ...
     `--pd-freeze` acima. O que sobra para este token: o preenchimento
  ...
  --pd-search-height: 60dvh;
  --pd-search-panel-width:      640px;
  --pd-search-panel-top:        54px;
  --pd-search-control-width:   300.75px;
  --pd-search-control-height:  36px;
  ...
  /* ---------------------------------------------------------------------------
     ...
     --------------------------------------------------------------------------- */
  --pd-card-collapse: 672px;   /* o `max-w-2xl` em que o `Columns` da âncora colapsa */
  --pd-card-min: calc((var(--pd-card-collapse) - 2 * var(--pd-space-4)) / 3);
...
@media (min-width: 997px) {
  :root {
    ...
    --pd-sticky-top: calc(var(--pd-navbar-height) + var(--pd-tabs-height));
  }
}
...
/* =============================================================================
   ...
   quadrada da luz (`--pd-glow-size`) e o par de amplitude da respiração
   (`--pd-glow-trough` e `--pd-glow-crest`). Ela era SEPARADA de propósito:
...
:root[data-theme] {
  ...
  /* O TOPO INTEIRO, não a linha 1: são os nove pontos do theme-classic que
     ...
     segunda linha entra. Ver `--pd-sticky-top`. */
  --ifm-navbar-height:                 var(--pd-sticky-top);
