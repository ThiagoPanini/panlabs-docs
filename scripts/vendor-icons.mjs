#!/usr/bin/env node
/**
 * Downloads icon artwork referenced by the manifest into `static/icons/`.
 *
 *   node scripts/vendor-icons.mjs            downloads what's missing
 *   node scripts/vendor-icons.mjs --conferir checks only, no download
 * Needs network access and runs manually, separate from the build: Lucide
 * renames glyphs between versions, so a missing name fails loudly here.
 */
...
// The ceiling is read from the manifest, not duplicated here: two ceilings
// could disagree, and there would be no way to tell which one was real.
...
const entries = [...source.matchAll(/\{nome: '([^']+)', papeis: \[([^\]]*)\][^}]*\}/g)].map((m) => ({
  ...
  // The upstream name only appears where it differs from ours.
...
// Section-to-icon pairs exist in three places: `SECTION_ICON_PAIRS` in the
// manifest, each category's `className` in the sidebar files, and the CSS
// mask rule. All three must agree, or the mismatch fails silently.
...
// Checking only the manifest and the CSS mask misses the sidebar files: a
// className renamed in a sidebar without touching the manifest doesn't break
// the build, doesn't remove the page, and doesn't drop the item from the
// list, it just silently loses its icon while the sidebar still looks right.
//
// The generated sidebar file below carries a `className` per page just like
// the hand-written ones, so leaving it out of the scan would blind one side
// of the check to its pairs.
const SIDEBARS = [
...
// The closing `'` is not decorative: `[a-z0-9-]+` matches a prefix, so
// `sidebar-icon--api-ownerX` would extract `api-owner` and the check would
// pass, believing the pair exists. Confirmed by writing that rename on purpose.
...
// The reverse direction catches renames the other way: a category asking for
// an icon the manifest doesn't know stays unmasked and silent.
...
if (process.argv[2] === '--conferir') {
  ...
  // The third side of the bijection: artwork in `static/icons/` nobody
  // declared. The React registry catches the other two sides; only this
  // script sees an orphan file, because nothing imports it.
...
for (const [name, upstream] of files) {
  ...
  // Lucide SVGs already ship ready to inherit color: `stroke="currentColor"`,
  // `fill="none"`, `viewBox="0 0 24 24"`. Nothing to rewrite.
