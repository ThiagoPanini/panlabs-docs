...
- Refusing from the compile pass the way the boot pass refuses — measured: the watcher never re-arms, so one unknown slug wedges the dev server until it is restarted.
- Watching the markdown tree as well, to catch the same edit — a directory created mid-session would still be missed, and the compile already runs on exactly the events that matter.
...
## The Root Is a Page, and the Anchor Doesn't Reach It

**This reopens a decision.** The landing was removed and the root turned into a jump by [A landing morre e a raiz redireciona para a primeira doc](https://github.com/ThiagoPanini/panlabs-docs/issues/94), which recorded that whether to have one again was a separate discussion, deliberately deferred. That discussion happened, and this is its outcome; the earlier decision isn't overwritten in silence, it's superseded here.

**Rule.** Make the root a real page with content of its own, never a redirect and never `noindex`. Let the anchor dictate the look inside the documentation only, and treat the root as free: the anchor ships no landing, so there's nothing there to depart from. Write the root in the first person, and scope that exception to the project's own routes rather than to the page — nothing under the published collection inherits it. Allow spectacle on this one route and nowhere else: it's where the movement vocabulary gets consumed, and it's the only place a photograph belongs. Say nothing on it that a growing site would make false — no count, no date, no status — and describe each navigation axis by the kind of record it holds, never by how much of it exists. Let this route, and only this route, change the chrome: reach it through a class the route writes on the document element, and move the top's height through the token the bridge already reads, never by writing a height.

**Why it binds.** The root was reachable and told the reader nothing: three redirect mechanisms dropped a first-time visitor into the middle of one CLI's reference with no chance to learn what the collection was. The anchor can't arbitrate that, because the anchor has no root page of its own — holding "any departure is a platform constraint" as absolute would have forced the site's front door to be justified by a system that never had one. Copy that describes a type instead of a volume is what lets every axis appear with the same weight while some of them are still empty, and it's also what keeps the page free of maintenance as the collection grows. Moving the top's height through the existing token is what keeps this at a class hook plus three rules: the bridge already writes the framework's navbar height from it, so one assignment collapses the bar, the sticky offset, and everything hanging off them, with no component wrapped or ejected.

**Refused, and why.**
- Keeping the anchor sentence absolute and naming the landing as an exception under it — an exception written on the most-looked-at page of the site is where a rule dies quietly.
- Keeping the voice rule absolute and writing the page in the third person about the collection — the headline doesn't survive the translation, and the alternative phrasing turns a page about why the collection exists into a tutorial preaching at the reader.
- Showing only the axes that already hold pages — structure is this repository's declared product and mocked content is its placeholder; hiding the empty axes would hide the product.
- Adopting the prototype's own information architecture — it invented axes the site doesn't have; changing the site's is a separate decision, not a side effect of a landing.
- Shipping the page with one movement token consumed and judging the rest later — two of the three tokens carry identical values, so the only thing that can justify keeping both names is each moving a different kind of thing, and that has to be decided while the page is being built.
- Keeping the tab strip on the root because every page sits under the same chrome — the strip exists to say which axis the reader is in, and on the root they're in none, so it would spend height and a hairline to inform nothing. The cost is registered: the chrome now varies by route, and that's an available precedent.
- A second image file per format, or a second file per colour mode — every veil over the photograph is made of the page surface, so one file already serves both modes, and a second format is weight for a page that has one image.
- Putting the display type step into the token scale — the step above the top of the scale has been asked for and refused three times; a single route derives it in its own scope instead.
- A scroll-driven reveal with no support guard — the fill mode holds the first keyframe, so where the feature is missing the section stays invisible for the life of the page and nothing in the build says so.

## The Blog Is a Tab, Not a Docs Instance

Written against [O Blog nasce como quinta aba, e não é uma instância de docs](https://github.com/ThiagoPanini/panlabs-docs/issues/194).

**This reopens a decision.** The contract below used to require exactly one tag from a closed set of three FORMATS — `novidades`, `tutoriais`, `notas` — fixed by that same issue. [As tags do blog viram assunto, e o índice ganha o desenho da prototipação 2a](https://github.com/ThiagoPanini/panlabs-docs/issues/203) removes that axis: a format tag classifies a text by how it was written, and what a reader looks for in a technical collection is what it is about. The type isn't overwritten in silence, it's superseded here, and the mechanism that enforced it is recorded under Refused. Everything else this section decides still binds.

**Rule.** Turn the blog on as the official `@docusaurus/plugin-content-blog`, already bundled with the installed preset, in one instance, content at `content/blog`, route `/blog`, no dependency added. Give it no recents sidebar (`blogSidebarCount: 0`), no archive route (`archiveBasePath: null`), no author page (`page: false` on the one author). One folder per article, `index.md` plus co-located images, permalink the slug with no date in the path. The body never opens with a Markdown `# title`: the title lives in front matter, same as every docs page, and a page component draws the `h1` from it. Every article carries at least one tag, every tag is a subject, and every one comes from the same closed catalog; the FIRST tag in the front matter is the accent the listing and the article page draw, which makes the array's order load-bearing. Check all of it once, when posts load, through the plugin's public `processBlogPosts` hook, and fail the build naming the file and the rule broken; let `onInlineTags: 'throw'` fail a tag the catalog doesn't list, the same native option a docs page can't offer since it has no tag system at all. Scope the house voice rule out of `content/blog/`: an article's voice is the author's choice, per article, and the zero-em-dash rule is the only prose rule that still reaches it. When the blog gets its own page components, reach the plugin through its four public component options (`blogListComponent` and the rest) from a folder outside `src/theme/`, never through a swizzle.

**Why it binds.** The plugin ships inside `@docusaurus/preset-classic`, already a dependency, so turning it on costs nothing new to install; a third-party blog plugin or a fifth `plugin-content-docs` instance would both buy less for a real cost. Requiring `title` in front matter and forbidding `# ` in the body isn't a style preference: the site's own `h1` registration (`src/theme/MDXComponents/index.js`) calls `useDoc()` to read the front matter and inject the subtitle beneath it, and that hook throws outside a docs `DocProvider`, a context blog posts don't have. A leading `# Title` also can't be caught by scanning `blogPost.content` inside `processBlogPosts`: `@docusaurus/utils`'s `removeContentTitle` runs unconditionally on every post and deletes exactly that line before the hook ever sees it, meant to backfill a missing `metadata.title`; since this project always supplies `title`, that deletion only hides the violation, so the check re-reads the file straight off disk instead. One axis is what keeps the reader's question and the site's taxonomy the same shape: a format tag answers "how was this written", and nobody browsing a technical collection asks that. Dropping it also removes the last reason for a list of tag names to live anywhere but `tags.yml` — `onInlineTags: 'throw'` closes the catalog natively, and with no membership rule of its own left to enforce, the load-time hook is down to checking that an article carries any tag at all. Promoting the first tag rather than adding a field is what keeps that reduction real: the accent slot the index and the article page draw has to be filled from somewhere, and the array is already ordered. Every Blog component the theme ships is `unsafe` on the swizzle ledger (`npx docusaurus swizzle @docusaurus/theme-classic --list`), the same class this repository holds at a zero budget, which is why custom pages, when they arrive, go through the plugin's public component options rather than a wrap or an eject: importing a theme component from an author's own file is use, not swizzle, the same argument that already lets the landing import `@theme/Layout`. The voice rule exists to keep documentation impersonal; a blog article is signed and dated by definition, so holding it to the same rule would produce prose that reads like documentation pretending to be a person.

**Refused, and why.**
- Swizzling any Blog component to reach the default layout faster — every one is `unsafe`, and the zero-`unsafe` budget doesn't carve out an exception for a plugin that's new to the site.
- A recents sidebar — the index and the tag pages already list every article chronologically; a sidebar would repeat that list in a second place that goes stale independently.
- A date segment in the permalink — makes the URL longer for no reader benefit and turns a slug rename into a link break the day the folder moves.
- Comments and a newsletter — both need an external service, and this site loads none by standing decision.
- A per-year archive page — only earns its cost once there's enough volume that pagination and tags stop being enough; this ticket's two seed articles aren't that.
- A page per author — the blog has exactly one author, and a page that lists one entry documents nothing a byline doesn't already say. `page: false` removes exactly that page; it doesn't reach the plugin's own `/blog/authors` list route, built unconditionally whenever `authors.yml` is non-empty (`createAuthorsRoutes` in `@docusaurus/plugin-content-blog`, no option gates it). That route stays: closing it needs a swizzle, and this decision already refuses that trade for a route nothing on the site links to.
- Reading `blogPost.content` alone to catch a leading `# title` — measured as insufficient: the plugin strips that exact line before the hook runs, which is the one case this rule most needs to catch.
- The three type tags, kept alongside the subjects — they would be a second axis wearing the first one's clothes, with their own routes, counts and descriptions, and a card has exactly one accent slot to spend. Their routes (`/blog/tags/novidades` and the other two) now 404, which is the accepted cost: nothing on the site linked to them and they had a day of life.
- A dedicated `topic:` front matter field to name the accent tag — the tag array's order already answers it, and a second mechanism for the same decision is one more thing to keep in step. The cost is registered: reordering an article's tags silently changes what its card shows, and nothing in the build says so.
- Hashing the slug into the generated plate's stripe angle or hue — a drawing nobody can predict from the source repaints itself the day a folder is renamed, and a listing page reads as a quilt instead of one surface.
